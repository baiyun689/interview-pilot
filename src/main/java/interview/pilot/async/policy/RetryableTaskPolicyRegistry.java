 ...
 * pre-refactor {@code claimKey} was a compile-time-exhaustive switch expression, and this
 * registry keeps the same exhaustive contract — the old reset if/else instead fell through
 * to a silent "treat as interview evaluation" branch, which would have mis-reset any future
 * type. {@link #forType} fails loudly; a test also enforces that every {@link AsyncTaskType}
 * constant is covered.
 */
@Component
