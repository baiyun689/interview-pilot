...
class KnowledgeGroundingTest {
  ...
  void disabledDirectiveDoesNotRetrieve() {
    ...
    var snapshot = grounding.ground(SCOPE, GroundingDirective.from(directive(false), List.of()));
  ...
  void preservesStableSourceMetadataAndRole() {
    ...
    var snapshot = grounding.ground(SCOPE, GroundingDirective.from(directive(true), List.of()));
