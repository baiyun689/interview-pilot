...
class AsyncTaskControllerTest {
  ...
  void setUp() {
    ...
    mvc = MockMvcBuilders.standaloneSetup(new AsyncTaskController(service))
  ...
  void exposesSafeTaskDtoAndAcceptsRetry() throws Exception {
    ...
    when(service.get(taskId)).thenReturn(failed);
    when(service.retry(org.mockito.ArgumentMatchers.eq(taskId), any(UUID.class)))
        .thenReturn(pending);
